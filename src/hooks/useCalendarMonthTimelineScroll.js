...
import { buildMonthWindow, computeMonthTimelineRowHeights, monthKey as monthKey2, CALENDAR_MONTH_WINDOW_NEAR_EDGE, computeMonthWindowExtension, monthWindowOffsetsKey, resolveMonthTimelineSpacing, resolveCardCenteredTimelineScrollOffset, CALENDAR_MONTH_TIMELINE_LIST_CONTENT_PADDING_BOTTOM, TIMELINE_DRAG_COMMIT_VY_PMS, TIMELINE_VIEWABILITY_CONFIG, scrollDragReleaseWillDecelerate, } from '../utils';
...
export function useCalendarMonthTimelineScroll({ currentDate, reduceMotion, isFocusedRef, deferredInteractionRef, initialVisibleMonth, windowWidth, listViewportHeight, fontScale, scrollY, screen = 'CalendarScreen', tabBarOnScrollBeginDrag, tabBarOnScrollEndDrag, tabBarOnMomentumScrollBegin, tabBarOnMomentumScrollEnd, }) {
    ...
    const recenterIndexRef = useRef(null);
    const monthListRef = useRef(null);
    const lastVisibleMonthKeyRef = useRef(null);
    const pendingMonthRef = useRef(null);
    const isUserScrollingRef = useRef(false);
    const pendingWindowExtendRef = useRef(null);
    const pendingFirstIndexRef = useRef(null);
    const layoutCoalesceRafRef = useRef(null);
    const pendingMeasuredInnerWRef = useRef(null);
    const monthPad = useMemo(() => resolveMonthTimelineSpacing({
        fontScale,
        windowWidth,
        listViewportHeight,
    ...
    const monthsData = useMemo(() => {
        ...
        if (perfProbe.enabled)
            perfProbe.setCulpritPhase(`${screen}.buildMonthWindow`);
    ...
    const timelineKey = useMemo(() => monthKey2(currentDate.getFullYear(), currentDate.getMonth()), [currentDate]);
    ...
    const commitVisibleMonth = useCallback((next, reason) => {
    ...
    const AnimatedFlashList = useMemo(() => Animated.createAnimatedComponent(FlashList), []);
    ...
    const estimatedCalendarInnerW = useMemo(() => Math.max(0, windowWidth - 2 * spacing[4] - 2 * monthPad.monthCardPadding), [windowWidth, monthPad.monthCardPadding]);
    const effectiveCalendarInnerW = measuredCalendarInnerW > 0 ? measuredCalendarInnerW : estimatedCalendarInnerW;
    const fullGridMetrics = useMemo(() => buildFullGridMetrics(effectiveCalendarInnerW), [effectiveCalendarInnerW]);
    ...
    const flushPendingMonth = useCallback(() => {
        ...
        if (extend && typeof idx === 'number') {
            if (perfProbe.enabled)
                perfProbe.setCulpritPhase(`${screen}.windowExtend`);
            deferredInteractionRef.current?.cancel();
            const task = InteractionManager.runAfterInteractions(() => {
                if (!isFocusedRef.current)
                    return;
                const { offsets, recenterIndex, didMutate: didMutateWindow } = computeMonthWindowExtension(windowOffsets, extend, idx);
    ...
    const onScrollBeginDrag = useCallback(() => {
        ...
        if (perfProbe.enabled)
            perfProbe.setCulpritPhase(`${screen}.scroll`);
    ...
    const onCalendarCardInnerLayout = useCallback((e) => {
    ...
    const timelineRowHeights = useMemo(() => computeMonthTimelineRowHeights({
        monthsData,
        fullGridMetrics,
    ...
    const scrollToCardCenteredMonth = useCallback((index) => {
        if (!listReadyRef.current || !isFocusedRef.current || listViewportHeight <= 0)
            return;
    ...
    const maybeRecenterAfterWindowChange = useCallback(() => {
        if (!listReadyRef.current || !isFocusedRef.current)
            return;
        ...
        const task = InteractionManager.runAfterInteractions(() => {
            if (!isFocusedRef.current)
                return;
            if (perfProbe.enabled)
                perfProbe.setCulpritPhase(`${screen}.recenter`);
