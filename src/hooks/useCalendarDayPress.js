...
export function useCalendarDayPress({ getEntryReqIdRef, setSelectedDate, setEditMood, setEditNote, setIsEditOpen, }) {
    return useCallback(async (isoDate) => {
        ...
        if (perfProbe.enabled)
            perfProbe.breadcrumb('calendar.dayTap');
        const tapStartMs = perfProbe.enabled ? perfProbe.nowMs() : 0;
        if (perfProbe.enabled)
            perfProbe.setCulpritPhase('CalendarScreen.dayTap');
        ...
        try {
            ...
            if (!isLatestRequest(getEntryReqIdRef, reqId))
                return;
