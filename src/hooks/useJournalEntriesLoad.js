...
import { getJournalEntriesSortedDescSnapshot, getEntriesSessionEpoch, peekJournalEntriesSortedDescFromSessionCache, } from '../storage';
import { entriesSameForJournal, isLatestRequest, nextRequestId } from '../utils';
import { logger } from '../security';
import { perfProbe } from '../perf';
function hydrateFromSessionPeek(setEntriesDesc, setJournalLoadCompleted) {
...
export function useJournalEntriesLoad(screen = 'JournalScreen') {
    const [entriesDesc, setEntriesDesc] = useState(() => {
        return peekJournalEntriesSortedDescFromSessionCache() ?? [];
    });
    const [journalLoadCompleted, setJournalLoadCompleted] = useState(() => peekJournalEntriesSortedDescFromSessionCache() !== undefined);
