...
import {
  ...
  peekHabitSelectionsForDateFromSessionCache,
  peekTrackedHabitIdsFromSessionCache,
} from '../storage';
...
function habitIdSetsEqual(a: ReadonlySet<HabitId>, b: ReadonlySet<HabitId>): boolean {
...
export function useTodayHabitStripModel(date: string): TodayHabitStripModel {
  ...
  const [selected, setSelected] = useState<ReadonlySet<HabitId>>(() => {
    const peeked = peekHabitSelectionsForDateFromSessionCache(date);
    return peeked ? new Set(peeked) : new Set();
  });
  const [tracked, setTracked] = useState<ReadonlySet<HabitId>>(() => {
  ...
  const loadTracked = useCallback(async () => {
    ...
    if (lastTrackedGenerationRef.current === generation && trackedLoadCountRef.current > 0) {
      if (mountedRef.current) setTrackedReady(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    const warmReturn =
      lastTrackedGenerationRef.current === trackedGen &&
