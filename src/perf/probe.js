...
let lastRafTs = null;
let culpritPhase = null;
let didWarnSlowFrameDuringCalendarScroll = false;
const SLOW_FRAME_WARN_THRESHOLD_MS = 16;
function isCalendarScrollPhase(phase) {
    return (phase === 'CalendarScreen.scroll' ||
...
function addBreadcrumb(name) {
    ...
    if (breadcrumbs.length > BREADCRUMB_LAST_N)
        breadcrumbs.splice(0, breadcrumbs.length - BREADCRUMB_LAST_N);
}
function inferPhaseForHitch(atMs) {
    const explicit = culpritPhase;
    if (explicit)
        return { phase: explicit, src: 'tag' };
    ...
    for (let i = breadcrumbs.length - 1; i >= 0; i--) {
        ...
        if (dt <= BREADCRUMB_INFER_WINDOW_MS)
            return { phase: b.name, src: 'crumb' };
        // Since we're walking backward, once we're beyond the window, older will also be beyond.
        if (b.atMs < atMs - BREADCRUMB_INFER_WINDOW_MS)
            break;
    }
...
const DEV_METRO_OR_GC_REPEAT_WINDOW_MS = 10000;
const DEV_METRO_OR_GC_REPEAT_COUNT = 3; // "repeats > 3 times" -> log on 4th
let lastHitchLogAtMs = 0;
const devMetroOrGcAtMs = [];
function shouldLogHitch(deltaMs, inferred, atMs) {
    // Rate-limit regardless of type.
    if (atMs - lastHitchLogAtMs < DEV_HITCH_LOG_MIN_INTERVAL_MS)
        return false;
    if (inferred.src === 'dev' || inferred.phase === 'DEV_METRO_OR_GC') {
        ...
        while (devMetroOrGcAtMs.length && devMetroOrGcAtMs[0] < atMs - DEV_METRO_OR_GC_REPEAT_WINDOW_MS)
            devMetroOrGcAtMs.shift();
...
function recordHitchAt(deltaMs, atMs, inferred) {
    ...
    if (hitchLast.length > HITCH_LAST_N)
        hitchLast.splice(0, hitchLast.length - HITCH_LAST_N);
...
function clearCulpritNextFrames(frames) {
    if (!PERF_ENABLED)
        return;
...
function tryInstallLongTaskObserver() {
    if (!PERF_ENABLED || longTaskObserverInstalled)
        return;
...
export const perfProbe = {
    ...
    init() {
        ...
        if (!didLogDeviceInfo) {
            ...
            logger.boot('perf.deviceInfo', {
                ...
                hermes: typeof globalThis.HermesInternal === 'object',
        ...
        if (!hitchRunning) {
            ...
            const loop = (ts) => {
                ...
                if (typeof lastRafTs === 'number') {
                    ...
                    if (!didWarnSlowFrameDuringCalendarScroll &&
     ...
     */
    setCulpritPhase(phase) {
        if (!PERF_ENABLED)
            return;
        culpritPhase = phase;
    },
    clearCulpritAfterFrames(frames) {
     ...
     */
    consumeMainTabPressToFocus(tab) {
        if (!PERF_ENABLED)
            return;
    ...
    measure(event, startMark, endMark, meta = {}) {
        if (!PERF_ENABLED)
            return;
    ...
    measureSince(event, startMs, meta = {}) {
        if (!PERF_ENABLED)
            return;
     ...
     */
    onScreenFocus(screenName) {
        if (!PERF_ENABLED)
            return;
        const ln = lastNav;
        if (!ln?.to || ln.to !== screenName)
            return;
    ...
    recordRenderCommit(id, phase, actualDurationMs, baseDurationMs) {
        if (!PERF_ENABLED)
            return;
    ...
    resetRenderStats(ids) {
        if (!PERF_ENABLED)
            return;
        ids.forEach((id) => renderStats.delete(id));
    },
    logRenderSummary(event, ids, meta = {}) {
        if (!PERF_ENABLED)
            return;
        const summary = ids
            .map((id) => {
