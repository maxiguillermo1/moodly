...
- **Calendar** screens load indexed entries + settings via **`fetchMoodCalendarSnapshot`** (single parallel read) where applicable; other screens use focused helpers (`getEntry`, `useMoodEntry`, etc.).
...
| **`calendar/screens/CalendarScreen.tsx`** | Month timeline (**Animated FlashList**); day tap modal; **`useTodayKey`**. |
