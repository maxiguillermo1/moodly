...
jest.mock('../../ui/Touchable', () => {
  ...
  return {
    Touchable: ({ children, onPress, ...rest }: { children?: React.ReactNode; onPress?: () => void }) =>
      React.createElement(Pressable, { onPress, accessibilityRole: 'button', ...rest }, children),
  };
});
...
describe('TodayGoalsExtension', () => {
  ...
  it('loads summaries via getTodayGoalSummaries only', async () => {
    render(<TodayGoalsExtension date="2026-05-12" />);
  ...
  it('navigates to Goals with the host day key', async () => {
    const { getByRole } = render(<TodayGoalsExtension date="2026-05-20" />);
    await waitFor(() => expect(goalsImpl.getTodayGoalSummaries).toHaveBeenCalled());
