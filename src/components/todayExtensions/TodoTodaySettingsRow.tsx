...
export type TodoTodaySettingsRowProps = {
  enabled: boolean;
...
export function TodoTodaySettingsRow({
  enabled,
  onToggle,
  isFirst = false,
  isLast = false,
}: TodoTodaySettingsRowProps): React.ReactElement {
  ...
  return (
    <GroupedRow
      symbol={{ name: 'alarm-outline', wellColor: s.indigo }}
      label="Reminders"
      showChevron={false}
      onPress={openTodo}
      accessibilityLabel="Open Reminders"
      accessibilityHint="Opens the reminders screen"
      isFirst={isFirst}
      isLast={isLast}
      right={(
        <Switch
          ...
          accessibilityLabel="Show Reminders section on Today"
