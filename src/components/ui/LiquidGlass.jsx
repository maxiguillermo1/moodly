...
export const LiquidGlass = React.memo(function LiquidGlass({ children, style, intensity = 56, tint = 'default', border = true, shadow = true, radius = sizing.capsuleRadius, prominent = false, }) {
    ...
    const secondaryBlurIntensity = useMemo(() => {
        if (!prominent || isDark || a11y.reduceTransparency || blurIntensity <= 0)
            return 0;
    ...
    const skipProminentBlurInDev = typeof __DEV__ !== 'undefined' &&
        __DEV__ &&
        prominent &&
        Platform.OS === 'ios';
    const showBlur = !!BlurViewAny && !a11y.reduceTransparency && blurIntensity > 0 && !skipProminentBlurInDev;
    ...
    const topEdgeOpacity = resolvedTint === 'dark' ? (prominent ? 0 : 0.12) : prominent ? 0.55 : 0.22;
    const topBandOpacity = resolvedTint === 'dark' ? (prominent ? 0 : 0.035) : prominent ? 0.16 : 0.065;
    const topBandHeight = resolvedTint === 'dark' ? (prominent ? 0 : 14) : prominent ? 20 : 14;
    const innerRimOpacity = prominent
        ...
        : resolvedTint === 'dark'
            ? 0.055
            : 0.1;
    return (_jsxs(View, { pointerEvents: "box-none", style: [containerStyle, style], children: [showBlur ? (_jsxs(_Fragment, { children: [_jsx(BlurViewAny, { tint: resolvedTint, intensity: blurIntensity, style: StyleSheet.absoluteFill }), secondaryBlurIntensity > 0 ? (_jsx(BlurViewAny, { tint: resolvedTint, intensity: secondaryBlurIntensity, style: StyleSheet.absoluteFill })) : null] })) : null, _jsx(View, { pointerEvents: "none", style: [StyleSheet.absoluteFill, { backgroundColor: fillBackground }] }), _jsxs(View, { pointerEvents: "none", style: StyleSheet.absoluteFill, children: [topEdgeOpacity > 0 ? (_jsx(View, { style: [
                            styles.topEdge,
                            { opacity: topEdgeOpacity },
                        ] })) : null, topBandHeight > 0 && topBandOpacity > 0 ? (_jsx(View, { style: [
                            styles.topBand,
                            {
                        ...
                        ] })) : null] }), border ? (_jsx(View, { pointerEvents: "none", style: [
