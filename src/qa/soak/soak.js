...
async function measure(state, name, op, kind = 'app') {
    let _a;
    ...
    try {
        ...
        const metric = ((_a = state.metrics)[name] || (_a[name] = { count: 0, totalMs: 0, maxMs: 0, slow: 0, kind }));
        metric.count += 1;
        metric.totalMs += ms;
...
function counts(entries, goals, tasks, taskHistory) {
    return {
...
async function currentCounts() {
    const entries = await readJson(ENTRIES_KEY, {});
    const goals = await getGoals();
    const tasks = await getTasks();
    const record = await readJson(TASKS_KEY, emptyTasksRecord());
...
function recordFailure(state, action, error) {
...
function createState() {
    return {
        ...
        metrics: {},
        memory: [],
        expectedEntryDates: new Set(),
        expectedCompletedTaskIds: new Set(),
        intentionallyDeletedTaskIds: new Set(),
        expectedArchivedGoalIds: new Set(),
    };
}
async function resetSessionCaches() {
...
async function bootstrapSettings(state) {
...
async function ensureGoalPool(state, loop) {
    ...
    const created = await measure(state, 'goals.create', () => upsertGoal({
...
async function simulateToday(state, date, loop) {
    ...
    if (reversibleGoal && goalHistoryCount < TARGET_GOAL_HISTORY) {
        ...
        await measure(state, 'goal.undoCompletion', () => upsertGoal({
...
async function simulateGoalsPage(state, date, loop) {
    ...
    await measure(state, 'goal.edit', () => upsertGoal({
        id: editable.id,
...
async function simulateTodoPage(state, date, loop) {
    await measure(state, 'screen.todo.load', () => getDayTodosForDate(date));
    if ((await getTasks()).length >= TARGET_TASKS)
        return;
...
async function simulateOtherScreens(state, date) {
...
function recurringTask(loop, date) {
    return {
...
async function triggerRecurrenceGeneration(state, date, loop) {
    await measure(state, 'recurrence.generate', async () => {
        ...
        if (currentTaskCount >= TARGET_TASKS || taskHistoryCount >= TARGET_TASK_HISTORY)
            return;
...
async function growDataVolume(state, loop) {
    ...
    for (let i = 0; i < Math.min(3, targetEntries); i += 1) {
        ...
        if (state.expectedEntryDates.has(date))
            continue;
        await upsertEntry({
            date,
...
async function validateData(state) {
    const entries = await readJson(ENTRIES_KEY, {});
    const goalsRaw = await readJson(GOALS_KEY, { version: 1, goalsById: {} });
    const tasksRaw = await readJson(TASKS_KEY, emptyTasksRecord());
    const errors = [];
    const goalIds = new Set();
    const taskIds = new Set();
    for (const date of state.expectedEntryDates) {
        if (!entries[date])
            errors.push(`lost mood/journal entry ${date}`);
    }
    for (const [date, entry] of Object.entries(entries)) {
        if (!isValidISODateKey(date) || entry.date !== date || typeof entry.note !== 'string')
            errors.push(`invalid entry ${date}`);
    }
    for (const [id, goal] of Object.entries(goalsRaw.goalsById ?? {})) {
        if (goalIds.has(id))
            errors.push(`duplicate goal id ${id}`);
        goalIds.add(id);
        if (goal.id !== id || !goal.title || !goal.progress || !Number.isFinite(goal.progress.currentValue))
            errors.push(`invalid goal ${id}`);
        for (const history of goal.history ?? []) {
            if (!history.id || !isValidISODateKey(history.date) || !Number.isFinite(history.value))
                errors.push(`invalid goal history ${goal.id}:${history.id}`);
        }
    }
    for (const id of state.expectedArchivedGoalIds) {
        const goal = goalsRaw.goalsById[id];
        if (goal && goal.status !== 'archived')
            errors.push(`archived goal reopened ${id}`);
    }
    for (const [id, task] of Object.entries(tasksRaw.tasksById ?? {})) {
        if (taskIds.has(id))
            errors.push(`duplicate task id ${id}`);
        taskIds.add(id);
        if (task.id !== id || !task.title || !['open', 'completed', 'archived'].includes(task.status))
            errors.push(`invalid task ${id}`);
        if (task.dueDate && !isValidISODateKey(task.dueDate))
            errors.push(`invalid task due date ${id}`);
        for (const subtask of task.subtasks ?? []) {
            if (!subtask.id || !subtask.title || typeof subtask.done !== 'boolean')
                errors.push(`orphaned/invalid subtask ${id}:${subtask.id}`);
        }
        for (const reminder of task.reminders ?? []) {
            if (!reminder.id || !isValidISODateKey(reminder.date) || !Number.isFinite(reminder.minutesFromMidnight))
                errors.push(`orphaned/invalid reminder ${id}:${reminder.id}`);
        }
        if (task.recurrence) {
            const r = task.recurrence;
            if (!r.id || !isValidISODateKey(r.startDate) || (r.endDate != null && !isValidISODateKey(r.endDate)))
                errors.push(`invalid recurrence ${id}`);
            if (!Number.isFinite(r.interval) || r.interval < 1)
                errors.push(`invalid recurrence interval ${id}`);
        }
    }
    for (const [taskId, history] of Object.entries(tasksRaw.historyByTaskId ?? {})) {
        if (!tasksRaw.tasksById[taskId])
            errors.push(`orphaned task history ${taskId}`);
        for (const event of history) {
            if (!event.id || event.taskId !== taskId || !Number.isFinite(event.at))
                errors.push(`invalid task history ${taskId}:${event.id}`);
        }
    }
    for (const id of state.expectedCompletedTaskIds) {
        const task = tasksRaw.tasksById[id];
        if (task && task.status !== 'completed')
            errors.push(`completed task reopened ${id}`);
    }
    for (const id of state.intentionallyDeletedTaskIds) {
        if (tasksRaw.tasksById[id])
            errors.push(`deleted task reappeared ${id}`);
    }
    if (errors.length > 0) {
        throw new Error(errors.slice(0, 20).join('\n'));
    }
}
function renderReport(state, status, dataCounts) {
    ...
    const metricRowsFor = (kind) => Object.entries(state.metrics)
...
async function writeReport(state, status) {
    let dataCounts;
    try {
        dataCounts = await currentCounts();
        for (const failure of state.failures)
            failure.counts = dataCounts;
    }
    catch {
...
describe('Kairo long-running soak test', () => {
    it('runs realistic storage/domain loops until stopped', async () => {
        ...
        const consoleSpies = process.env.SOAK_VERBOSE === '1'
            ? []
            : [
        ...
        if (CLEANUP && fs.existsSync(REPORT_PATH))
        ...
        try {
            while (!stopRequested && (MAX_LOOPS == null || state.loops < MAX_LOOPS)) {
                ...
                try {
                    ...
                    if (!HOT_PATH_ONLY)
                        await measure(state, 'data.validate', () => validateData(state), 'harness');
                }
                catch (error) {
            ...
            if (MAX_LOOPS != null && state.loops >= MAX_LOOPS)
                state.stopReason = `SOAK_MAX_LOOPS=${MAX_LOOPS}`;
        }
        finally {
            state.memory.push(memorySnapshot());
