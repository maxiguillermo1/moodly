...
async function pushOperation(userId, op) {
    ...
    switch (op.kind) {
        ...
        case 'tracked_habits_snapshot': {
            const { error } = await client.from('tracked_habits').upsert({ user_id: userId, habit_ids: op.habitIds, updated_at_ms: Date.now() }, { onConflict: 'user_id' });
            if (error)
                throw error;
            return;
        }
        case 'goals_snapshot': {
            ...
            if (gErr)
                throw gErr;
            const progressRows = goals.flatMap((g) => (g.history ?? []).map((h) => ({
                user_id: userId,
                goal_id: g.id,
                date: h.date,
                value: h.value,
        ...
        case 'settings_snapshot': {
            const { error } = await client.from('app_settings').upsert({ user_id: userId, settings_json: op.settings, updated_at_ms: Date.now() }, { onConflict: 'user_id' });
            if (error)
                throw error;
            return;
        }
        case 'tasks_snapshot': {
            const { error } = await client.from('tasks_records').upsert({ user_id: userId, payload_json: op.record, updated_at_ms: Date.now() }, { onConflict: 'user_id' });
            if (error)
                throw error;
            return;
        }
...
export async function pushOutboxToCloud(user) {
