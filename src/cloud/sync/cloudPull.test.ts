...
function queryChain(result: QueryResult, mode: 'list' | 'single' = 'list') {
...
function noopApplier(overrides: Partial<CloudPullApplier> = {}): CloudPullApplier {
  return {
...
describe('pullCloudDataToLocal', () => {
  ...
  it('merges cloud moods with local LWW before applyMoodEntries', async () => {
    ...
    await pullCloudDataToLocal({ id: 'user-1' } as never, applier);

    expect(applyMoodEntries).toHaveBeenCalledTimes(1);
    const callArgs = (applyMoodEntries as jest.Mock).mock.calls[0] as [Record<string, { mood: string }>];
  ...
  it('applies habit selections from cloud rows', async () => {
    ...
    await pullCloudDataToLocal({ id: 'user-1' } as never, noopApplier({ applyHabitSelections }));
