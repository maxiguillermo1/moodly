...
const session = { user: { id: 'user-reinstall' } } as Session;
...
describe('runFreshSignInFlow', () => {
  ...
  it('pulls cloud before checking local data (reinstall path)', async () => {
    const order: string[] = [];
    (registerKairoCloudPullApplier as jest.Mock).mockImplementation(async () => {
      order.push('registerApplier');
    });
    (runInitialCloudRestore as jest.Mock).mockImplementation(async () => {
      order.push('pullRestore');
    });
    (deviceHasLocalJournalData as jest.Mock).mockImplementation(async () => {
  ...
  it('does not enqueue destructive snapshots when local journal is empty', async () => {
    (deviceHasLocalJournalData as jest.Mock).mockResolvedValue(false);
  ...
  it('uploads local snapshot only after pull when device has journal data', async () => {
    ...
    (enqueueFullLocalSnapshotForCloud as jest.Mock).mockImplementation(async () => {
      order.push('enqueueSnapshot');
    });
    (pushOutboxToCloud as jest.Mock).mockImplementation(async () => {
