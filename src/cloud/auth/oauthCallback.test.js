...
describe('parseOAuthCallbackUrl', () => {
    ...
    it('reads tokens from hash fragment', () => {
        const params = parseOAuthCallbackUrl('kairo://auth/callback#access_token=at&refresh_token=rt');
        expect(params.accessToken).toBe('at');
        expect(params.refreshToken).toBe('rt');
    });
    it('reads OAuth error from callback', () => {
        const params = parseOAuthCallbackUrl('kairo://auth/callback?error=access_denied&error_description=User%20denied');
