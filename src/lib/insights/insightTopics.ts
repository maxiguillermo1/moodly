...
import type { InsightArtifact } from '../../types/insights.types';

/**
 * Stable topic key for an artifact (same observation family ⇒ same cooldown bucket
 * unless habit id splits co-occurrence).
 */
export function topicIdForArtifact(artifact: InsightArtifact): string {
...
export function continuityMoodFamily(artifact: InsightArtifact): boolean {
  return (
    artifact.messageKey === 'insights.streak.mood_logging' || artifact.messageKey === 'insights.week.full_mood_week'
  );
}
