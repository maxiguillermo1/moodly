...
export function addLocalDays(dayKey, deltaDays) {
    if (!isValidLocalCalendarDayKey(dayKey))
        return dayKey;
...
export function startOfMonthContaining(dayInMonth) {
    if (!isValidLocalCalendarDayKey(dayInMonth))
        return dayInMonth;
    return `${dayInMonth.slice(0, 7)}-01`;
}
/** Last calendar day of the month containing `dayInMonth`. */
export function endOfMonthContaining(dayInMonth) {
    if (!isValidLocalCalendarDayKey(dayInMonth))
        return dayInMonth;
