...
function idFor(start, key, suffix) {
    return `${start}:${key}:${suffix}`;
}
function push(out, start, key, tone, priority, suffix, params) {
    out.push({
...
function pickCooccurrenceHabit(activities) {
    ...
    for (const def of HABIT_CATALOG) {
        ...
        for (const d of days) {
            ...
            if (!row)
                continue;
            if (!row.habits.selectedIds.includes(def.id))
                continue;
            if (isMoodAtLeastB(row.mood.grade))
                co += 1;
        }
        if (co >= 3)
            return { habitId: def.id, habitLabel: def.label, coDays: co };
    }
    return null;
}
...
export function runReflectionEngine(input) {
    const { period, activities, metrics, priorMetrics } = input;
    const start = period.start;
    const out = [];
    const totallyEmpty = metrics.daysWithMoodEntry === 0 &&
    ...
    const priorHasSignal = priorMetrics &&
    ...
    if (co) {
        const key = co.coDays >= 5 ? 'insights.habit.mood_cooccurrence_soft' : 'insights.habit.mood_cooccurrence_tentative';
