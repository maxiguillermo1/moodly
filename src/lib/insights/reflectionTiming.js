 ...
 */
export const EMPTY_TIMING_STATE = Object.freeze({
    schemaVersion: 1,
    topicLastSurfacedAtMs: {},
});
/** Cooldowns are conservative: calm product, not engagement loops. */
export const TOPIC_COOLDOWN_MS = Object.freeze({
...
function cooldownMsForTopic(topicId) {
    if (topicId.startsWith(HABIT_CO_PREFIX))
        return 5 * 24 * 60 * 60 * 1000;
    return TOPIC_COOLDOWN_MS[topicId] ?? 48 * 60 * 60 * 1000;
}
function isAlwaysAllowedDuringCooldown(q) {
    return q.messageKey === 'insights.empty.week_gentle';
}
/**
 * Drops insights whose topic was surfaced recently (read-only).
 */
export function filterInsightsByCooldowns(insights, state, nowMs) {
    const out = [];
    for (const q of insights) {
        ...
        if (last !== undefined && nowMs - last < cd)
            continue;
        out.push(q);
    }
    return out;
 ...
 */
export function timingStateAfterRecording(prev, topicIds, nowMs) {
