...
import {
  CALENDAR_MONTH_TIMELINE_CARD_CENTER_TUNING_PX,
  CALENDAR_MONTH_TIMELINE_CARD_TOP_Y_RATIO,
  CALENDAR_MONTH_TIMELINE_INTER_GRID_GAP,
} from './constants';
import {
  maxTimelineMonthCardHeight,
  minMonthSectionBottomForCardCenterClip,
  resolveMonthSectionTopForViewportCenteredCard,
  timelineMonthSectionCardTop,
} from './timelineMonthRowMetrics';

export type MonthTimelineSpacing = {
...
export function resolveMonthTimelineSpacing(params: {
  ...
  const innerW = Math.max(
    0,
    params.windowWidth - horizontalPadding - 2 * base.monthCardPadding
  );
  const grid = buildFullGridMetrics(innerW);
  const cardHeight = maxTimelineMonthCardHeight(grid, base.monthCardPadding);
  const monthSectionTop = resolveMonthSectionTopForViewportCenteredCard(
    viewportH,
    cardHeight,
    CALENDAR_MONTH_TIMELINE_CARD_CENTER_TUNING_PX,
    CALENDAR_MONTH_TIMELINE_CARD_TOP_Y_RATIO
  ...
  return {
    monthSectionTop,
    monthSectionBottom: Math.max(
      base.monthSectionBottom,
      minBottom + CALENDAR_MONTH_TIMELINE_INTER_GRID_GAP
    ),
