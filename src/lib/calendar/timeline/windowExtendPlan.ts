...
export type TimelineWindowOffsets = { start: number; end: number };

export type TimelineWindowExtendPlan =
  | { kind: 'unchanged' }
  | {
      kind: 'extend';
      offsets: TimelineWindowOffsets;
      windowKey: string;
...
export function planTimelineWindowExtend(args: {
  ...
  if (edge === 'start') {
    ...
    if (windowKey === lastWindowKey) return { kind: 'unchanged' };
    return {
      kind: 'extend',
