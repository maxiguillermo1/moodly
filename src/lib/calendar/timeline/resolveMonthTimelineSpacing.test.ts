...
describe('resolveMonthTimelineSpacing', () => {
  it('places the card high in the viewport when the row top is aligned', () => {
    ...
    expect(resolved.monthSectionTop).toBe(
      resolveMonthSectionTopForViewportCenteredCard(
        listViewportHeight,
        cardH,
        CALENDAR_MONTH_TIMELINE_CARD_CENTER_TUNING_PX,
    ...
    const rowH = getTimelineMonthRowMetrics(
      2025,
      0,
      grid,
      resolved.monthCardPadding,
      resolved.monthSectionTop,
      resolved.monthSectionBottom
    ).rowHeight;
