...
export function readAnchorDateFromRouteParams(params) {
    const p = params;
    const y = p?.year;
    const m = p?.month;
    const date = p?.date;
    ...
    const hasValid = typeof y === 'number' &&
        Number.isFinite(y) &&
        typeof m === 'number' &&
        Number.isFinite(m) &&
        m >= 0 &&
    ...
    return hasValid ? new Date(y, m, 1) : new Date(deviceToday.getFullYear(), deviceToday.getMonth(), 1);
}
export function readInitialSelectedDayKeyFromRoute(params, anchor) {
    const p = params;
    const y = p?.year;
    const m = p?.month;
    const date = p?.date;
