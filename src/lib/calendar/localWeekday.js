...
export function localFirstWeekdayOfMonth(year, monthIndex0) {
 ...
 */
export function localWeekdayForMonthDay(year, monthIndex0, day) {
 ...
 */
export function fillLocalWeekdaysForMonthDays1to31(year, monthIndex0, out) {
    const first = localFirstWeekdayOfMonth(year, monthIndex0);
    for (let d = 1; d <= 31; d++) {
        out[d] = (first + d - 1) % 7;
    }
