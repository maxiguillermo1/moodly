...
export function entriesSameForJournal(prev, next) {
    ...
    for (let i = 0; i < prev.length; i += 1) {
        ...
        if (!a || !b)
            return false;
        if (a.date !== b.date || a.updatedAt !== b.updatedAt || a.mood !== b.mood || a.note !== b.note)
            return false;
    }
    return true;
}
export function buildWeekdaySections(entries) {
    const buckets = [[], [], [], [], [], [], []];
...
export function buildMonthSections(entries) {
    const map = new Map();
    for (let i = 0; i < entries.length; i += 1) {
        const e = entries[i];
...
export function buildMoodSections(entries) {
    const map = new Map();
...
export function applySoloMonthFilter(base, soloKey) {
    if (soloKey === null)
        return base;
...
export function applySoloWeekdayFilter(base, soloWeekday) {
    if (soloWeekday === null)
        return base;
...
export function applySoloMoodFilter(base, soloGrade) {
    if (soloGrade === null)
        return base;
...
export function buildJournalSectionsForViewMode(viewMode, entriesDesc, solo) {
    switch (viewMode) {
        case 'byMonth':
...
export function a11yAnnouncementForViewMode(mode) {
    switch (mode) {
        case 'newest':
...
export function nextMoodSoloFromHeaderTap(fromGrade, currentSolo, moodsWithEntries) {
    if (moodsWithEntries.length === 0)
        return currentSolo;
    if (currentSolo === null)
        return 'A+';
    if (moodsWithEntries.length === 1)
        return null;
    ...
    for (let step = 1; step <= MOOD_GRADES.length; step += 1) {
        ...
        if (moodsWithEntries.includes(g))
            return g;
    }
    return moodsWithEntries[0];
}
export function nextWeekdaySoloFromHeaderTap(fromW, currentSolo, weekdaysWithEntries) {
    if (weekdaysWithEntries.length === 0)
        return currentSolo;
    if (weekdaysWithEntries.length === 1) {
        return currentSolo === null ? weekdaysWithEntries[0] : null;
    }
    const start = JOURNAL_WEEKDAY_SECTION_ORDER.indexOf(fromW);
    ...
    for (let step = 1; step <= JOURNAL_WEEKDAY_SECTION_ORDER.length; step += 1) {
        ...
        if (weekdaysWithEntries.includes(w))
            return w;
    }
    return weekdaysWithEntries[0];
}
export function nextMonthSoloFromHeaderTap(fromMonthKey, currentSolo, monthKeysWithEntries) {
