...
export const JOURNAL_WEEKDAY_SECTION_ORDER = [0, 1, 2, 3, 4, 5, 6] as const;

export function mondayFirstWeekdayIndex0FromDayKey(dayKey: string): number | null {
...
export type JournalEntryPresence = {
  moodsWithEntries: MoodGrade[];
  weekdaysWithEntries: number[];
  monthKeysWithEntries: string[];
  monthKeysSet: ReadonlySet<string>;
 ...
 */
export function scanJournalEntryPresence(entries: readonly MoodEntry[]): JournalEntryPresence {
  const moodSet = new Set<MoodGrade>();
