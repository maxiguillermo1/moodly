 ...
 */
import { canonicalizeGoalModel, computeGoalProgress, deriveCurrentValueFromHistory, goalLoggedDayCount, goalProgressPercent, goalsDisplaySame, mergeGoalHistoryByDate, } from './goalMath';
...
describe('mergeGoalHistoryByDate', () => {
    it('treats undefined or null history as empty', () => {
        expect(mergeGoalHistoryByDate(undefined)).toEqual([]);
        expect(mergeGoalHistoryByDate(null)).toEqual([]);
    });
...
describe('large merged history', () => {
    it('stays linear-time for many rows', () => {
        ...
        const t0 = Date.now();
        const m = mergeGoalHistoryByDate(h);
        canonicalizeGoalModel(baseGoal({
            history: m,
            progress: { currentValue: 0, targetValue: 10, unit: '', frequency: 'none' },
        }));
