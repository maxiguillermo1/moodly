 ...
 */
export function mergeGoalHistoryByDate(history) {
...
export function goalLoggedDayCount(history) {
...
function sumHistoryValues(history) {
    let s = 0;
    for (const item of history) {
        const v = item.value;
 ...
 */
export function deriveCurrentValueFromHistory(type, mergedHistory) {
 ...
 */
export function canonicalizeGoalModel(goal) {
...
export function goalCompletedOnDate(goal, date = getToday()) {
    const c = canonicalizeGoalModel(goal);
    return historyOnDate(c.history, date).some((item) => item.value > 0);
}
export function goalStreak(goal, today = getToday()) {
    const c = canonicalizeGoalModel(goal);
    const completed = completedDateSet(c.history);
    return goalStreakFromCompleted(completed, today);
}
function goalStreakFromCompleted(completed, today) {
...
function insightMessage(goal, streak, percent) {
    let message = 'Start with one small step.';
    if (goal.status === 'completed')
        message = 'You completed this goal gently and steadily.';
    else if (streak >= 2)
        message = `You've completed this ${streak} days in a row.`;
    else if (percent > 0)
        message = `You're ${percent}% of the way there.`;
    else if (goal.type === 'project')
        message = 'One clear milestone is enough to begin.';
    return message;
}
export function buildGoalInsight(goal, today = getToday()) {
    const c = canonicalizeGoalModel(goal);
...
export function computeGoalProgress(goal, today = getToday()) {
    const c = canonicalizeGoalModel(goal);
    const completed = completedDateSet(c.history);
...
export function sortGoalsForDisplay(goals) {
    return [...goals].sort((a, b) => {
        const statusRank = (g) => (g.status === 'active' ? 0 : g.status === 'completed' ? 1 : 2);
...
export function goalsDisplaySame(prev, next) {
...
export function todayGoalPreview(goals) {
