...
function idFor(windowStart: string, key: NarrativeMessageKey, suffix: string): string {
  return `${windowStart}:${key}:${suffix}`;
}

function confidenceForChapter(ch: TimelineChapter): NarrativeConfidence {
  if (ch.spanDays >= 42 && ch.activeDays >= 18) return 'high';
  if (ch.spanDays >= 21 && ch.activeDays >= 8) return 'medium';
  return 'low';
}

function chapterMessageKey(kind: TimelineChapter['phaseKind']): NarrativeMessageKey | null {
  switch (kind) {
    case 'steady':
 ...
 */
export function runNarrativeEngine(input: NarrativeEngineInput): NarrativeBundle {
  ...
  let moodDays = 0;
  for (const r of rows) {
    if (r.hasActivity) activeDays += 1;
    if (r.journalNote) journalDays += 1;
    if (r.moodPresent) moodDays += 1;
  }
  ...
  for (const ch of chapterCandidates) {
    if (chapterArtifacts >= 2) break;
    const mk = chapterMessageKey(ch.phaseKind);
    if (!mk) continue;
    const conf = confidenceForChapter(ch);
    if (conf === 'low' && rows.length >= 56 && chapterCandidates.length > 1) continue;
  ...
  const summaryConf: NarrativeConfidence = spanDays >= 56 ? 'medium' : 'low';
  const summary: NarrativeArtifact = {
...
export function stampNarrativeBundle(bundle: NarrativeBundle, composedAt: number): NarrativeBundle {
