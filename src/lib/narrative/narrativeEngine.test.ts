...
function activeRow(date: string): DayActivity {
  return {
    ...baseRow(date),
...
function journalRow(date: string): DayActivity {
  return {
    ...activeRow(date),
...
function fillRange(start: string, n: number, fn: (d: string) => DayActivity): Record<string, DayActivity> {
...
describe('narrativeEngine', () => {
  it('is deterministic for the same window + activities', () => {
    const window: NarrativeWindow = { start: '2026-03-01', end: '2026-03-28' };
  ...
  it('emits a transition when a quiet fortnight precedes a more active one', () => {
    const window: NarrativeWindow = { start: '2026-04-01', end: '2026-04-28' };
  ...
  it('detects journal rebuild signal across halves', () => {
    const window: NarrativeWindow = { start: '2026-05-01', end: '2026-06-14' };
    const activities: Record<string, DayActivity> = {};
    let d = '2026-05-01';
    for (let i = 0; i < 45; i++) {
  ...
  it('handles large synthetic windows without throwing', () => {
    ...
    const window: NarrativeWindow = { start, end };
    const bundle = runNarrativeEngine({ window, activities });
  ...
  it('always ends with gentle span overview', () => {
    const window: NarrativeWindow = { start: '2026-07-01', end: '2026-07-14' };
