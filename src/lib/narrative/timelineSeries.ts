...
import type { PhaseKind } from '../../types/narrative.types';
...
export function classifyPhase(metrics: Omit<FortnightBucket, 'phaseKind' | 'start' | 'end'>): PhaseKind {
  const { spanDays, activeDays, journalDays } = metrics;
  if (spanDays <= 0) return 'steady';
  const ratio = activeDays / spanDays;
  const jr = journalDays / spanDays;
  if (activeDays === 0) return 'sparse';
  if (ratio <= 0.18) return 'sparse';
  if (jr >= Math.max(0.32, 4 / spanDays)) return 'reflective';
  if (ratio >= 0.52) return 'high_activity';
  if (ratio <= 0.26) return 'quiet';
  return 'steady';
}

function foldBucket(rows: readonly DaySignalRow[], start: LocalDateKey, end: LocalDateKey): Omit<FortnightBucket, 'phaseKind' | 'start' | 'end'> {
  let activeDays = 0;
  let moodDays = 0;
  let journalDays = 0;
  let sumMoodRank = 0;
  ...
  let weekdayActive = 0;
  let weekdayTotal = 0;
  const map = new Map(rows.map((r) => [r.date, r] as const));
  let spanDays = 0;
  let d = start;
  for (let g = 0; g < 32; g++) {
    spanDays += 1;
    const row = map.get(d);
    if (row) {
      if (row.hasActivity) activeDays += 1;
      if (row.moodPresent) moodDays += 1;
      if (row.moodRank != null) {
        sumMoodRank += row.moodRank;
        moodSamples += 1;
      }
      if (row.journalNote) journalDays += 1;
      if (row.isWeekend) {
        weekendTotal += 1;
        if (row.hasActivity) weekendActive += 1;
      } else {
        weekdayTotal += 1;
        if (row.hasActivity) weekdayActive += 1;
      }
    }
    if (d >= end) break;
    d = addLocalDays(d, 1);
 ...
 */
export function buildFortnightBuckets(windowStart: LocalDateKey, windowEnd: LocalDateKey, rows: readonly DaySignalRow[]): FortnightBucket[] {
  const out: FortnightBucket[] = [];
