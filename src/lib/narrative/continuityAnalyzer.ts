...
export type ContinuitySplit = {
  firstJournalDays: number;
  secondJournalDays: number;
  /** True when the back half shows meaningfully more journal presence without diagnosing “why”. */
  journalRebuildSignal: boolean;
};

export function analyzeContinuity(rows: readonly DaySignalRow[]): ContinuitySplit {
  if (rows.length < 14) {
