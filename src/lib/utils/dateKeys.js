...
export function dayOfMonthInCalendarMonth(dayKey, year, monthIndex0) {
    if (!isValidLocalDayKeyFormat(dayKey))
        return 0;
    const want = `${year}-${String(monthIndex0 + 1).padStart(2, '0')}`;
    if (!dayKey.startsWith(want + '-'))
        return 0;
    const d = Number(dayKey.slice(8, 10));
    return Number.isFinite(d) && d >= 1 && d <= 31 ? d : 0;
}
export function localDayKeyFromParts(year, monthIndex0, day) {
