...
- **Schema / migrations** — `ensureLocalPersistenceReady()` (see `src/data/persistence/bootstrap.ts`). Before each version step, **`writePreMigrationBackup`** stores **`kairo.migrationBackup.<k>_to_<k+1>.<ts>`** (`migrationBackup.ts`).
- **Internal export (no UI)** — `buildKairoLocalExportV1` + validators in `src/data/persistence/localExport/kairoLocalExport.ts` (requires `getAllKeys` on the `KeyValueStore` adapter).
- **Reminders (per-day list)**: **`kairo.dayTodos`** — **`getDayTodosForDate`**, **`addDayTodo`**, **`setDayTodoDone`**, **`setDayTodoReminder`**, **`deleteDayTodo`**, **`reorderOpenDayTodos`**, **`clearCompletedDayTodos`** (`src/data/storage/dayTodosStorage.ts`; types in **`src/types/todo.types.ts`**).
