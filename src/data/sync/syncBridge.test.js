...
import { resetAuthSessionCacheForTests, setCachedAuthSession, } from '../../cloud/auth/authSessionCache';
...
describe('syncBridge', () => {
    beforeEach(() => {
        ...
        isCloudPullActive.mockReturnValue(false);
        restoreAuthSession.mockResolvedValue({ user: { id: 'user-1' } });
    });
    it('skips enqueue and sync trigger while cloud pull is active', () => {
        isCloudPullActive.mockReturnValue(true);
    ...
    it('uses cached session without SecureStore read on debounced sync', async () => {
        jest.useFakeTimers();
        setCachedAuthSession({ user: { id: 'user-cached' } });
        notifyMoodEntryUpserted(sampleEntry);
        await Promise.resolve();
        await Promise.resolve();
        await Promise.resolve();
