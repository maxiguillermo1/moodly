 ...
 */
const HABIT_SELECTIONS_KEY = 'kairo.habitSelections';
const DAY_TODOS_KEY = 'kairo.dayTodos';
const dayShardKey = (date) => `kairo.tasks.day.${date}`;
describe('extension persistence vs settings toggles', () => {
    ...
    it('disabling habits in settings does not erase saved day selections', async () => {
        const habitSelections = require('./habitSelectionsStorage');
    ...
    it('corrupt per-day habit selections are quarantined and reset safely', async () => {
        ...
        expect(await AsyncStorage.getItem(HABIT_SELECTIONS_KEY)).toBe(JSON.stringify({ v: 3, selections: {}, toggleTotals: {} }));
        const keys = await AsyncStorage.getAllKeys();
        expect(keys.some((k) => k.startsWith(`${HABIT_SELECTIONS_KEY}.corrupt.`))).toBe(true);
    });
    it('disabling reminders in settings does not erase saved day todos', async () => {
        const dayTodos = require('./dayTodosStorage');
        const settingsStorage = require('./settingsStorage');
        ...
        expect(parsed.some((item) => item.title === 'water plants')).toBe(true);
    });
    ...
    it('migrates v2 disk to v3 and drops legacy onCounts (counts come from selections only)', async () => {
        const mod = require('@react-native-async-storage/async-storage');
        const AsyncStorage = mod?.default ?? mod;
        await AsyncStorage.setItem(HABIT_SELECTIONS_KEY, JSON.stringify({
            v: 2,
