...
import { clearHabitSelectionsOnDisk, loadHabitSelectionsJsonFromDisk, persistHabitSelectionsJsonToDisk, quarantineRawHabitSelectionsJson, } from './habitSelectionsBackend';
...
function diskVersionIs(value, expected) {
    if (value === expected)
        return true;
    if (typeof value === 'string' && Number(value) === expected)
        return true;
    if (typeof value === 'number' && Number.isFinite(value) && Math.floor(value) === expected)
        return true;
    return false;
}
function normalizeDaySelections(ids) {
...
function orderHabitIdsForPersist(ids) {
    const set = new Set(ids);
    return HABIT_IDS.filter((id) => set.has(id));
}
function parseSelectionsRoot(raw) {
    ...
    for (const [k, v] of Object.entries(raw)) {
        ...
        if (normalized && normalized.length > 0)
            selections[k] = orderHabitIdsForPersist(normalized);
    }
    return selections;
}
function legacyToggleTotalsNonEmpty(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return false;
...
function serializeCanonicalSelections(selections) {
 ...
 */
function deriveMarkedDayCountsFromSelections(selections) {
    const counts = {};
...
function cloneTrustedBundle(bundle) {
    const selections = {};
    for (const [k, ids] of Object.entries(bundle.selections)) {
        if (ids.length > 0)
            selections[k] = ids.slice();
    }
...
async function getMutableBundleForWrite() {
    if (cache)
        return cloneTrustedBundle(cache);
    if (loadPromise)
        return cloneTrustedBundle(await loadPromise);
    return cloneTrustedBundle(await readFromDisk());
}
async function persistBundle(next) {
    await ensureLocalPersistenceReady();
    assertLocalPersistenceWritable();
...
export async function getHabitSelectionsForDate(date) {
...
export async function getHabitMarkedDayCounts() {
    const all = await getBundle();
    return deriveMarkedDayCountsFromSelections(all.selections);
}
export async function toggleHabitForDate(date, habitId) {
    if (!isValidISODateKey(date)) {
        throw new Error(`[habitSelectionsStorage] Invalid date key: ${String(date)}`);
    }
    ...
    return withWriteLock(async () => {
        ...
        if (next.length === 0)
            delete all.selections[date];
        else
            all.selections[date] = next;
...
export async function removeHabitFromDate(date, habitId) {
    if (!isValidISODateKey(date)) {
        throw new Error(`[habitSelectionsStorage] Invalid date key: ${String(date)}`);
    }
    if (!isHabitId(habitId)) {
 ...
 */
export async function clearAllHabitSelections() {
    return withWriteLock(async () => {
        try {
            await ensureLocalPersistenceReady();
            assertLocalPersistenceWritable();
...
export async function setAllHabitSelectionsRecord(next) {
    return withWriteLock(async () => {
        try {
 ...
 */
export async function getHabitSelectionsRecordSnapshot() {
    try {
        ...
        for (const [k, v] of Object.entries(b.selections)) {
            ...
            if (!Array.isArray(v) || v.length === 0)
                continue;
            out[k] = orderHabitIdsForPersist(v);
        }
        return out;
    }
    catch (e) {
...
export function resetHabitSelectionsStorageSessionStateForTests() {
    invalidateHabitSelectionsSessionCache();
}
export function invalidateHabitSelectionsSessionCache() {
    cache = null;
    loadPromise = null;
    cacheGeneration = 0;
...
export function peekHabitSelectionsForDateFromSessionCache(date) {
    if (!isValidISODateKey(date))
        return [];
    if (!cache)
        return undefined;
...
export function getHabitSelectionsCacheGeneration() {
