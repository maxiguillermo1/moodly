...
describe('moodStorage reliability edge cases', () => {
  ...
  it('large entries map round-trips after session cache reset (simulated relaunch)', async () => {
    ...
    const N = 600;
    const bulk: Record<string, { date: string; mood: 'B'; note: string; createdAt: number; updatedAt: number }> = {};
  ...
  it('returns defensive copies so callers cannot mutate entry caches', async () => {
    const { upsertEntry, getEntry, getAllEntries, getAllEntriesWithMonthIndex, getEntriesSortedDesc, getMoodStats } =
      require('./moodStorage') as typeof import('./moodStorage');
  ...
  it('keeps warmed derived caches correct across upsert and delete', async () => {
    const {
      upsertEntry,
      deleteEntry,
      getAllEntriesWithMonthIndex,
      getEntriesSortedDesc,
      getMoodStats,
      warmEntriesSessionCaches,
    } = require('./moodStorage') as typeof import('./moodStorage');
  ...
  it('keeps calendar snapshot month references stable until affected months change', async () => {
    const { upsertEntry, getCalendarEntriesByMonthIndexSnapshot } =
      require('./moodStorage') as typeof import('./moodStorage');
    await upsertEntry({ date: '2026-05-01', mood: 'A', note: '', createdAt: 1, updatedAt: 1 });
    await upsertEntry({ date: '2026-06-01', mood: 'B', note: '', createdAt: 1, updatedAt: 1 });
  ...
  it('rapid sequential upserts same date serialize; final mood wins', async () => {
    const { upsertEntry, getEntry } = require('./moodStorage') as typeof import('./moodStorage');
    const mod: any = require('@react-native-async-storage/async-storage');
    const AsyncStorage: any = mod?.default ?? mod;
    await AsyncStorage.clear();
  ...
  it('upsertEntry throws on impossible calendar day (no partial write)', async () => {
    ...
    await AsyncStorage.clear();
    await expect(
      upsertEntry({ date: '2026-02-30', mood: 'A', note: '', createdAt: 1, updatedAt: 1 })
    ).rejects.toThrow(/Invalid ISO date key/);
    const all = await getAllEntries();
  ...
  it('upsertEntry throws on invalid mood (no partial write)', async () => {
    ...
    await AsyncStorage.clear();
    await expect(
      upsertEntry({ date: '2026-02-09', mood: 'Z' as any, note: '', createdAt: 1, updatedAt: 1 })
    ).rejects.toThrow(/Invalid mood grade/);
    const all = await getAllEntries();
    expect(Object.keys(all)).toHaveLength(0);
  });
});
