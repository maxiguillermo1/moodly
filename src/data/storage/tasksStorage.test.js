...
describe('tasksStorage foundation', () => {
    ...
    it('migrates legacy day todos once and preserves day API behavior', async () => {
        ...
        await AsyncStorage.setItem(LEGACY_KEY, JSON.stringify({
            '2026-09-01': [
                { id: 'legacy-a', title: 'legacy task', done: false, createdAt: 1, sortIndex: 0, reminderMinutes: 540 },
            ],
        }));
        const tasks = require('./tasksStorage');
    ...
    it('prevents duplicate recurrence dates from generating by contract helper', () => {
        const { nextRecurrenceDate } = require('../../lib/todos/taskModel');
        const rule = {
            id: 'r',
            frequency: 'weekdays',
            interval: 1,
    ...
    it('handles weekly weekdays and month-end recurrence deterministically', () => {
        const { nextRecurrenceDate } = require('../../lib/todos/taskModel');
        expect(nextRecurrenceDate({ id: 'r', frequency: 'weekly', interval: 1, startDate: '2026-05-11', endDate: null, weekdays: [1, 3], lastGeneratedDate: null }, '2026-05-11')).toBe('2026-05-13');
        expect(nextRecurrenceDate({ id: 'm', frequency: 'monthly', interval: 1, startDate: '2026-01-31', endDate: null, weekdays: [], lastGeneratedDate: null }, '2026-01-31')).toBe('2026-02-28');
    });
