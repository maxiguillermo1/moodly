describe('goalsStorage foundation', () => {
    ...
    it('creates goals and returns defensive copies', async () => {
        const goals = require('./goalsStorage');
    ...
    it('migrates v1 disk record to v2 and snapshots pre-migration JSON', async () => {
        ...
        await AsyncStorage.setItem('kairo.goals', JSON.stringify({
            version: 1,
            goalsById: {
                walk: {
                    id: 'walk',
        ...
        expect(keys.some((k) => k.startsWith('kairo.goals.migrate_backup.'))).toBe(true);
    });
    it('does not re-backup when record is already v2', async () => {
        ...
        await goals.getGoals();
        const keys = await AsyncStorage.getAllKeys();
        expect(keys.some((k) => k.startsWith('kairo.goals.migrate_backup.'))).toBe(false);
    });
});
