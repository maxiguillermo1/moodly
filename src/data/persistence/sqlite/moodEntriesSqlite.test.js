...
describe('sqlite mood entries persistence', () => {
    ...
    it('imports legacy AsyncStorage blob into mood_entries on bootstrap', async () => {
        ...
        const { countMoodEntries, loadAllMoodEntriesFromSqlite } = require('./moodEntriesStore');
    ...
    it('upsert and delete single rows without rewriting full blob', async () => {
        const { ensureLocalPersistenceReady } = require('../bootstrap');
        await ensureLocalPersistenceReady();
        const { upsertEntry, getEntry, deleteEntry, getAllEntries } = require('../../storage/moodStorage');
        await upsertEntry({
            date: '2026-03-01',
            mood: 'B',
            note: 'sqlite row',
    ...
    it('preserves write-lock serialization under sqlite backend', async () => {
        const { ensureLocalPersistenceReady } = require('../bootstrap');
        await ensureLocalPersistenceReady();
        const { upsertEntry, getAllEntries } = require('../../storage/moodStorage');
        await Promise.all([
