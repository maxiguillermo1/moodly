...
export async function readSqlUserVersion(db) {
    const row = await db.getFirstAsync('PRAGMA user_version');
    return row?.user_version ?? 0;
}
async function writeSqlUserVersion(db, version) {
    await db.execAsync(`PRAGMA user_version = ${version}`);
}
export async function runSqlMigrations(db) {
    ...
    if (diskVersion > CURRENT_SQL_SCHEMA_VERSION) {
        ...
        throw new Error(`[runSqlMigrations] Disk SQL schema ${diskVersion} is newer than app schema ${CURRENT_SQL_SCHEMA_VERSION}`);
    }
