 ...
 */
function splitParams(source, params) {
...
export function createInMemoryKairoDatabase() {
    ...
    const upsertRow = (tableName, row, pkCol) => {
    ...
    const db = {
        async execAsync(source) {
            ...
            for (const stmt of statements) {
                if (/^PRAGMA\s+journal_mode/i.test(stmt))
                    continue;
            ...
            if (insertMatch) {
                ...
                if (/INSERT OR REPLACE/i.test(normalized)) {
                    if (tableName === 'habit_selections') {
                        ...
                        const pkCol = tableName === 'mood_entries' ? 'date' : tableName === 'goals' ? 'id' : 'key';
        ...
        async getAllAsync(source, ...params) {
            ...
            if (/^PRAGMA\s+user_version\s*$/i.test(normalized)) {
                return [{ user_version: meta.get('user_version') ?? 0 }];
            }
            const countMatch = normalized.match(/^SELECT COUNT\(\*\)\s+as\s+cnt\s+FROM\s+(\w+)\s*$/i);
            if (countMatch) {
                return [{ cnt: ensureTable(countMatch[1]).length }];
            }
            ...
            if (selectAll) {
                return [...ensureTable(selectAll[1])];
            }
            return [];
        },
...
export function getSharedInMemoryKairoDatabase() {
    if (!sharedTestDb)
        sharedTestDb = createInMemoryKairoDatabase();
    return sharedTestDb;
}
export function resetSharedInMemoryKairoDatabase() {
    sharedTestDb = createInMemoryKairoDatabase();
}
