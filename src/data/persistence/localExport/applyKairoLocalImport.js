...
import { assertLocalPersistenceWritable, ensureLocalPersistenceReady, resetPersistenceBootstrapSession, } from '../bootstrap';
...
function isBlockedImportKey(key) {
    if (key.includes('.corrupt.'))
        return true;
    if (key.startsWith('kairo.migrationBackup.'))
        return true;
    return false;
}
/**
 ...
 */
export async function applyKairoLocalImportV1(payload) {
    ...
    for (const [key, value] of Object.entries(payload.kv)) {
        if (typeof key !== 'string' || !key.startsWith('kairo.'))
            continue;
