...
export class DiskSchemaAheadOfAppError extends Error {
    constructor(diskVersion, appVersion) {
...
export async function runLocalMigrations(store) {
...
export async function resetCorruptSchemaMeta(store) {
    const raw = await store.getItem('kairo.schemaMeta');
    if (typeof raw === 'string' && raw.length > 0) {
        const m = await readSchemaMeta(store);
        if (!m)
            await quarantineSchemaMeta(store, raw);
    }
}
