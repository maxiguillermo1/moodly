...
function monthKeyFromDateKey(date) {
    return date.length >= 7 ? date.slice(0, 7) : '';
}
function safeNormalizeNotePreview(note) {
...
function buildMetadata(date, warnings, rangeTruncated) {
    return {
        composedAt: Date.now(),
        date,
        rangeTruncated,
...
function moodSectionFromEntry(entry) {
    if (!entry)
        return emptyMood();
    return {
        present: true,
...
function journalSectionFromEntry(entry) {
    if (!entry)
        return emptyJournal();
...
function calendarSection(date, entry) {
    return {
...
function mapReminderItems(items) {
    return items.map((t) => ({
        id: t.id,
...
function canonicalizeGoalsForReadModel(goals) {
    const out = [];
...
function goalsSectionForDate(goalsCanon, date) {
    const items = [];
    let n = 0;
    for (const g of goalsCanon) {
        if (g.status !== 'active')
            continue;
        if (n >= MAX_ACTIVE_GOALS_PER_DAY)
            break;
        n += 1;
...
function summaryFromParts(args) {
    ...
    const counts = {
        mood: args.mood.hasEntry ? 1 : 0,
        journalNote: args.journal.present ? 1 : 0,
        habits: habitsCount,
        goalsWithProgress,
        reminders: args.reminders.items.length,
    };
    const hasAnyActivity = counts.mood > 0 ||
...
function buildDayActivityCore(date, entry, habitSelected, trackedIds, goalsCanon, reminders, warnings, rangeTruncated) {
    ...
    const rem = { items: mapReminderItems(reminders) };
...
export function emptyDayActivity(date, warning) {
    const dk = date;
    return {
        date: dk,
        mood: emptyMood(),
...
function compareIsoDateKeys(a, b) {
    if (a === b)
        return 0;
    return a < b ? -1 : 1;
}
 ...
 */
export function enumerateLocalDateRangeInclusive(start, end) {
    if (!isValidISODateKey(start) || !isValidISODateKey(end))
        return null;
    const warnings = [];
...
export async function getDayActivityRange(startDate, endDate) {
    await ensureLocalPersistenceReady();
    ...
    try {
        ...
        dates.forEach((date, i) => {
            ...
            out[date] = buildDayActivityCore(date, entry, habitSelected, trackedIds, goalsCanon, reminders, mergedWarnings, truncated || undefined);
        });
    }
    catch (error) {
