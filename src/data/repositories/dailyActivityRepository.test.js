...
describe('dailyActivityRepository', () => {
    beforeEach(async () => {
        ...
        const goals = require('../storage/goalsStorage');
        const habits = require('../storage/habitSelectionsStorage');
        const habitTrack = require('../storage/habitTrackingStorage');
        const tasks = require('../storage/tasksStorage');
    ...
    it('returns safe empty activity for invalid date', async () => {
        const { getDayActivity } = require('./dailyActivityRepository');
    ...
    it('getDayActivityRange batches mood via one getAllEntries', async () => {
        ...
        const { getDayActivityRange } = require('./dailyActivityRepository');
    ...
    it('enumerateLocalDateRangeInclusive returns null for invalid keys', () => {
        const { enumerateLocalDateRangeInclusive } = require('./dailyActivityRepository');
        expect(enumerateLocalDateRangeInclusive('x', '2026-01-01')).toBeNull();
    });
    it('truncates very long ranges', () => {
        const { enumerateLocalDateRangeInclusive, MAX_DAILY_ACTIVITY_RANGE_DAYS } = require('./dailyActivityRepository');
    ...
    it('getTodayActivity uses local today key', async () => {
        const { getTodayActivity } = require('./dailyActivityRepository');
    ...
    it('read path does not write mood entries', async () => {
        ...
        const { getDayActivity, getDayActivityRange } = require('./dailyActivityRepository');
    ...
    it('skips goals that fail canonicalization without losing mood or valid goals', async () => {
        ...
        const spy = jest.spyOn(goals, 'getGoals').mockResolvedValueOnce([{ id: 'broken' }, ...stored]);
        const { getDayActivity } = require('./dailyActivityRepository');
