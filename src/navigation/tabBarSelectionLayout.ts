...
export function computeTabSelectionLayout(params: {
  ...
  const slotStart = (i: number) => TAB_SELECTION_TRACK_PAD_H + i * (slotW + TAB_SELECTION_SLOT_GAP);
...
export function computeAllTabSelectionLayouts(
  trackInnerW: number,
  nTabs: number,
  minStadiumW: number
): Array<{ x: number; w: number }> {
  if (trackInnerW <= 0 || nTabs <= 0) return [];
  return Array.from({ length: nTabs }, (_, index) =>
    computeTabSelectionLayout({ trackInnerW, nTabs, index, minStadiumW })
  );
}
