...
import { View, Text, StyleSheet, AccessibilityInfo, Dimensions, } from 'react-native';
...
import { MONTH_NAMES_EN_LONG, monthKey as monthKey2, formatDateToISO, CALENDAR_MONTH_TIMELINE_HEADER_CHROME, CALENDAR_MONTH_TIMELINE_LIST_PULL_UP_PX, CALENDAR_MONTH_TIMELINE_TITLE_CARD_GAP_PX, } from '@/utils';
import { logger } from '@/security';
import { PerfProfiler, usePerfScreen, perfProbe } from '@/perf';
import { useScrollDrivenTabBarVisibility, useShowTabBarOnScreenBlur, useMoodCalendarSnapshotLoad, useCalendarMonthTimelineScroll, useCalendarEntryEdit, } from '@/hooks';
...
export default function CalendarScreen() {
     ...
     */
    const initialAnchorDateRef = useRef(null);
    const initialAnchorKeyRef = useRef(null);
    const initialSelectedDateRef = useRef(null);
    if (!initialAnchorDateRef.current) {
        const y = route.params?.year;
        const m = route.params?.month;
        const hasValidParams = typeof y === 'number' &&
            Number.isFinite(y) &&
            typeof m === 'number' &&
            Number.isFinite(m) &&
            m >= 0 &&
    ...
    const [currentDate] = useState(() => initialAnchorDateRef.current);
    const [reduceMotion, setReduceMotion] = useState(false);
    const rm = reduceMotion || appTheme.a11y.reduceMotion;
    const { showTabBar, onScrollBeginDrag: tabBarOnScrollBeginDrag, onScrollEndDrag: tabBarOnScrollEndDrag, onMomentumScrollBegin: tabBarOnMomentumScrollBegin, onMomentumScrollEnd: tabBarOnMomentumScrollEnd, } = useScrollDrivenTabBarVisibility();
    ...
    const deferredInteractionRef = useRef(null);
    const estimatedListViewportHeight = useMemo(() => {
        const windowH = appTheme.windowHeight > 0 ? appTheme.windowHeight : Dimensions.get('window').height;
    ...
    const listViewportHeight = measuredListViewportHeight > 0 ? measuredListViewportHeight : estimatedListViewportHeight;
    const scrollY = useSharedValue(0);
    const { visibleMonth, monthsData, timelineKey, initialMonthIndex, monthListRef, viewabilityConfig, onViewableItemsChanged, onListLayout, onScrollBeginDrag, onMomentumScrollBegin, onScrollEndDrag, onMomentumScrollEnd, AnimatedFlashList, keyExtractor, fullGridMetrics, monthTimelineSpacing: monthPad, onCalendarCardInnerLayout, overrideItemLayout, clearLayoutCoalesce, } = useCalendarMonthTimelineScroll({
        currentDate,
        reduceMotion: rm,
        isFocusedRef,
        deferredInteractionRef,
        initialVisibleMonth: {
            y: initialAnchorDateRef.current.getFullYear(),
            m: initialAnchorDateRef.current.getMonth(),
        },
    ...
    const onTimelineListLayout = useCallback((e) => {
    ...
    const { entriesByMonthKey, setEntriesByMonthKey, calendarMoodStyle, entriesRevisionRef, mountedRef: isMountedRef, } = useMoodCalendarSnapshotLoad({
    ...
    const { selectedDate, selectedDateRef, isEditOpen, editMood, editNote, isSaving, setEditMood, setEditNote, closeEdit, handlePressDate, handleSave, } = useCalendarEntryEdit({
    ...
    const calLimits = useMemo(() => getCalendarTextLimits(appTheme.fontScale, appTheme.windowWidth), [appTheme.fontScale, appTheme.windowWidth]);
    const monthListSurfaceStyle = useMemo(() => ({ flex: 1, backgroundColor: sys.secondaryBackground }), [sys.secondaryBackground]);
    ...
    const renderMonthItem = useCallback(({ item }) => {
        ...
        return (_jsx(CalendarTimelineMonth, { item: item, monthEntries: monthEntries, entriesRevision: entriesListEpoch, selectedForThisMonth: selectedForThisMonth, monthSectionTopPad: monthPad.monthSectionTop, monthSectionBottomPad: monthPad.monthSectionBottom, monthCardPadding: monthPad.monthCardPadding, fullGridMetrics: fullGridMetrics, calendarMoodStyle: calendarMoodStyle, todayKey: todayKey, onPressDate: handlePressDate, reduceMotion: rm, onHapticSelect: handleHapticSelect, moodGradeColorStyle: moodGradeColorStyle, isDark: isDark, onCalendarCardInnerLayout: onCalendarCardInnerLayout, monthSectionTitleMaxFontMult: calLimits.monthSectionTitle, calendarListEpoch: calendarListEpoch, styles: timelineMonthStyles }));
    }, [
        calendarListEpoch,
        calendarMoodStyle,
    ...
    return (_jsxs(SafeAreaView, { style: styles.container, edges: ['top'], children: [_jsxs(View, { style: styles.topBar, children: [_jsx(CapsuleButton, { kind: "back", iconName: "chevron-back", iconColor: sys.blue, label: String(visibleMonth.y), labelColor: sys.blue, onPress: () => navigation.navigate('CalendarView', { year: visibleMonth.y }), accessibilityLabel: "Back to year view" }), _jsx(View, { style: styles.topBarRight, children: _jsx(CapsuleButton, { kind: "icon", iconName: "settings-outline", iconColor: sys.label, onPress: () => navigation.navigate('Settings'), accessibilityLabel: "Settings" }) })] }), _jsx(View, { style: styles.largeTitleContainer, children: _jsx(Animated.View, { style: largeTitleStyle, children: _jsx(Text, { style: styles.largeMonthTitle, allowFontScaling: true, maxFontSizeMultiplier: calLimits.monthSectionTitle, children: MONTH_NAMES_EN_LONG[visibleMonth.m] }) }) }), _jsx(PerfProfiler, { id: "list.calendarMonthTimeline", children: _jsx(View, { style: monthTimelineLiftStyle, children: _jsx(AnimatedFlashList
                    // Key remount keeps initialScrollIndex deterministic when we reset the anchor (Today).
                    , { ref: monthListRef, style: monthListSurfaceStyle, data: monthsData, keyExtractor: keyExtractor, 
                        ...
                        drawDistance: 500, initialNumToRender: 2, maxToRenderPerBatch: 2, updateCellsBatchingPeriod: 80, 
                        // Phase 3: isolate scroll-path work; allow native to clip offscreen views.
                        removeClippedSubviews: true, overrideItemLayout: overrideItemLayout, onLayout: onTimelineListLayout, initialScrollIndex: initialMonthIndex, showsVerticalScrollIndicator: false, contentContainerStyle: styles.monthTimeline, onScrollBeginDrag: onScrollBeginDrag, onMomentumScrollBegin: onMomentumScrollBegin, onScrollEndDrag: onScrollEndDrag, onMomentumScrollEnd: onMomentumScrollEnd, onScroll: scrollHandler, scrollEventThrottle: 16, keyboardShouldPersistTaps: "handled", viewabilityConfig: viewabilityConfig, onViewableItemsChanged: onViewableItemsChanged, renderItem: renderMonthItem, extraData: monthListExtraData }, timelineKey) }) }), _jsx(CalendarEditModal, { visible: isEditOpen, selectedDate: selectedDate, editMood: editMood, editNote: editNote, isSaving: isSaving, setEditMood: setEditMood, setEditNote: setEditNote, onCancel: closeEdit, onSave: () => {
                    void handleSave();
                } })] }));
}
