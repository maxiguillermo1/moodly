...
export default function AccountLoginScreen() {
    ...
    const { initialized, signInEmail, signUpEmail, signInApple, signInGoogle, continueOffline } = useAuth();
    ...
    if (!initialized) {
        return (_jsx(SafeAreaView, { style: [styles.container, styles.loading], edges: ['top', 'bottom'], children: _jsx(ActivityIndicator, { color: s.blue, accessibilityLabel: "Loading welcome screen" }) }));
    }
    return (_jsxs(SafeAreaView, { style: styles.container, edges: ['top', 'bottom'], children: [_jsx(ScreenHeader, { title: "Welcome", showSettings: false }), _jsx(KeyboardAvoidingView, { style: styles.container, behavior: Platform.OS === 'ios' ? 'padding' : undefined, keyboardVerticalOffset: Platform.OS === 'ios' ? 8 : 0, children: _jsx(ScrollView, { contentContainerStyle: styles.scroll, keyboardShouldPersistTaps: "handled", children: _jsx(AccountAuthForm, { variant: "gate", onContinueOffline: handleContinueOffline, continuingOffline: continuingOffline, onSignInEmail: signInEmail, onSignUpEmail: signUpEmail, onSignInApple: signInApple, onSignInGoogle: signInGoogle }) }) })] }));
}
