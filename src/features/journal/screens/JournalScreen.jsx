...
import { View, Text, FlatList, StyleSheet, Alert, SectionList, ActivityIndicator, } from 'react-native';
...
import { formatDateForDisplay, isValidLocalCalendarDayKey, monthNameLongEn, buildMonthSections, buildWeekdaySections, buildMoodSections, applySoloMonthFilter, applySoloWeekdayFilter, applySoloMoodFilter, a11yAnnouncementForViewMode, scanJournalEntryPresence, EMPTY_JOURNAL_ENTRY_PRESENCE, nextMoodSoloFromHeaderTap, nextWeekdaySoloFromHeaderTap, nextMonthSoloFromHeaderTap, JOURNAL_WEEKDAY_SECTION_NAMES, } from '@/utils';
 ...
 */
const JOURNAL_LIST_IMPL = 'flashlist';
export default function JournalScreen() {
    ...
    const listSurfaceStyle = useMemo(() => ({ flex: 1, backgroundColor: s.background }), [s.background]);
    ...
    const [byMoodSoloGrade, setByMoodSoloGrade] = useState(null);
    /** By month: `null` = all months; otherwise only that `YYYY-MM` section (tap month header to cycle). */
    const [byMonthSoloKey, setByMonthSoloKey] = useState(null);
    /** By weekday: `null` = all days; 0–6 = Mon–Sun solo (tap header to cycle). */
    const [byDaySoloWeekday, setByDaySoloWeekday] = useState(null);
    const [editingEntry, setEditingEntry] = useState(null);
    const [editMood, setEditMood] = useState(null);
    const [editNote, setEditNote] = useState('');
    const { entriesDesc, journalLoadCompleted, mountedRef, focusedRef, reload: reloadJournalEntries } = useJournalEntriesLoad();
    const flatEntries = useMemo(() => {
        if (viewMode === 'oldest') {
            ...
            for (let i = 0; i < n; i += 1)
                out[i] = entriesDesc[n - 1 - i];
            return out;
        }
        return entriesDesc;
    }, [entriesDesc, viewMode]);
    const isGroupedView = viewMode === 'byMonth' || viewMode === 'byDay' || viewMode === 'byMood';
    const journalEntryPresence = useMemo(() => (isGroupedView ? scanJournalEntryPresence(entriesDesc) : EMPTY_JOURNAL_ENTRY_PRESENCE), [entriesDesc, isGroupedView]);
    const monthSectionsBase = useMemo(() => (viewMode === 'byMonth' ? buildMonthSections(entriesDesc) : []), [entriesDesc, viewMode]);
    const weekdaySectionsBase = useMemo(() => (viewMode === 'byDay' ? buildWeekdaySections(entriesDesc) : []), [entriesDesc, viewMode]);
    const moodSectionsBase = useMemo(() => (viewMode === 'byMood' ? buildMoodSections(entriesDesc) : []), [entriesDesc, viewMode]);
    const journalSectionsForList = useMemo(() => {
        switch (viewMode) {
            case 'byMonth':
    ...
    useEffect(() => {
        if (viewMode !== 'byMonth' || byMonthSoloKey === null)
            return;
        if (!journalEntryPresence.monthKeysSet.has(byMonthSoloKey))
            setByMonthSoloKey(null);
    }, [byMonthSoloKey, journalEntryPresence.monthKeysSet, viewMode]);
    useEffect(() => {
        if (viewMode !== 'byDay' || byDaySoloWeekday === null)
            return;
        if (!journalEntryPresence.weekdaySet.has(byDaySoloWeekday))
            setByDaySoloWeekday(null);
    }, [byDaySoloWeekday, journalEntryPresence.weekdaySet, viewMode]);
    const advanceByMoodHeaderTap = useCallback((fromGrade) => {
    ...
    const advanceByWeekdayHeaderTap = useCallback((fromW) => {
        const { weekdaysWithEntries } = journalEntryPresence;
        if (weekdaysWithEntries.length === 0)
            return;
        haptics.select();
    ...
    const clearByDaySolo = useCallback(() => {
        if (byDaySoloWeekday === null)
            return;
        haptics.select();
        setByDaySoloWeekday(null);
        announceForAccessibility('Showing all weekday groups');
    }, [byDaySoloWeekday]);
    const advanceByMonthHeaderTap = useCallback((fromMonthKey) => {
        const { monthKeysWithEntries } = journalEntryPresence;
        if (monthKeysWithEntries.length === 0)
            return;
        haptics.select();
    ...
    const selectViewMode = useCallback((next) => {
        if (next === viewMode) {
    ...
    const renderSortHeader = useCallback(() => {
        if (entriesDesc.length === 0)
            return null;
        return (_jsxs(View, { style: styles.sortHeader, children: [_jsx(Touchable, { onPress: toggleSortMenu, accessibilityRole: "button", accessibilityLabel: "Sort", accessibilityHint: "Shows journal ordering and grouping options", accessibilityState: { expanded: sortMenuOpen }, children: _jsxs(View, { style: styles.sortTriggerRow, children: [_jsx(Text, { style: styles.sortTriggerLabel, allowFontScaling: true, maxFontSizeMultiplier: 1.3, children: "Sort" }), _jsx(Ionicons, { name: sortMenuOpen ? 'chevron-up' : 'chevron-down', size: 18, color: s.secondaryLabel, accessibilityElementsHidden: true, importantForAccessibility: "no-hide-descendants" })] }) }), (viewMode === 'newest' || viewMode === 'oldest') && !sortMenuOpen ? (_jsxs(Text, { style: styles.journalFlatEntryTotal, allowFontScaling: true, maxFontSizeMultiplier: 1.34, children: [entriesDesc.length, " ", entriesDesc.length === 1 ? 'entry' : 'entries'] })) : null, sortMenuOpen ? (_jsx(View, { style: styles.sortMenuCard, accessibilityViewIsModal: true, children: JOURNAL_VIEW_OPTIONS.map((opt, idx) => {
                        const selected = viewMode === opt.mode;
                        const isLast = idx === JOURNAL_VIEW_OPTIONS.length - 1;
                        return (_jsx(Touchable, { onPress: () => selectViewMode(opt.mode), style: [
                                styles.sortMenuRow,
                                isLast && styles.sortMenuRowLast,
                                selected && styles.sortMenuRowSelected,
                            ], accessibilityRole: "button", accessibilityLabel: opt.label, accessibilityState: { selected }, children: _jsx(Text, { style: [styles.sortMenuRowLabel, !selected && styles.sortMenuRowLabelMuted], allowFontScaling: true, maxFontSizeMultiplier: 1.32, children: opt.label }) }, opt.mode));
                    }) })) : null] }));
    }, [entriesDesc.length, selectViewMode, sortMenuOpen, styles, s.secondaryLabel, toggleSortMenu, viewMode]);
    const handleTapEntry = useCallback((entry) => {
        haptics.select();
    ...
    const handleLongPressEntry = useCallback((entry) => {
        Alert.alert('Delete Entry', `Delete your entry for ${formatDateForDisplay(entry.date)}?`, [
    ...
    const renderEntry = useCallback(({ item }) => {
        return (_jsx(JournalEntryRow, { entry: item, moodGradeColorStyle: moodGradeColorStyle, isDark: isDark, onTap: handleTapEntry, onLongPress: handleLongPressEntry, rowStyles: journalRowStyles }));
    }, [handleLongPressEntry, handleTapEntry, isDark, journalRowStyles, moodGradeColorStyle]);
    const renderJournalSectionHeader = useCallback(({ section }) => {
        const shellStyle = [
        ...
        if (section.kind === 'mood' && section.moodGrade) {
            ...
            return (_jsx(View, { style: shellStyle, children: _jsx(Touchable, { onPress: () => advanceByMoodHeaderTap(g), onLongPress: clearByMoodSolo, accessibilityRole: "button", accessibilityLabel: formatMoodA11yLabel(g), accessibilityHint: moodSectionHeaderA11yHint, children: _jsxs(View, { children: [_jsx(Text, { style: styles.journalSectionCaptionBold, allowFontScaling: true, maxFontSizeMultiplier: 1.3, children: section.title }), _jsx(Text, { style: styles.journalSectionMeta, allowFontScaling: true, maxFontSizeMultiplier: 1.34, children: section.subtitle })] }) }) }));
        }
        if (section.kind === 'day' && section.weekdayIndex0 !== undefined) {
            const w = section.weekdayIndex0;
            return (_jsx(View, { style: shellStyle, children: _jsx(Touchable, { onPress: () => advanceByWeekdayHeaderTap(w), onLongPress: clearByDaySolo, accessibilityRole: "button", accessibilityLabel: JOURNAL_WEEKDAY_SECTION_NAMES[w], accessibilityHint: "Tap to hide other weekdays and show the next day. Long press to show all weekdays.", children: _jsxs(View, { children: [_jsx(Text, { style: styles.journalSectionCaptionBold, allowFontScaling: true, maxFontSizeMultiplier: 1.3, children: section.title }), _jsx(Text, { style: styles.journalSectionMeta, allowFontScaling: true, maxFontSizeMultiplier: 1.34, children: section.subtitle })] }) }) }));
        }
        if (section.kind === 'month' && section.monthKey) {
            ...
            return (_jsx(View, { style: shellStyle, children: _jsx(Touchable, { onPress: () => advanceByMonthHeaderTap(mk), onLongPress: clearByMonthSolo, accessibilityRole: "button", accessibilityLabel: a11yMonth, accessibilityHint: "Tap to hide other months and show the next month. Long press to show all months.", children: _jsxs(View, { children: [_jsx(Text, { style: styles.journalSectionCaptionBold, allowFontScaling: true, maxFontSizeMultiplier: 1.3, children: section.title }), _jsx(Text, { style: styles.journalSectionMeta, allowFontScaling: true, maxFontSizeMultiplier: 1.34, children: section.subtitle })] }) }) }));
        }
        return (_jsxs(View, { style: shellStyle, children: [_jsx(Text, { style: styles.journalSectionCaption, allowFontScaling: true, maxFontSizeMultiplier: 1.3, children: section.title }), _jsx(Text, { style: styles.journalSectionMeta, allowFontScaling: true, maxFontSizeMultiplier: 1.34, children: section.subtitle })] }));
    }, [
    ...
    const renderEmptyState = useCallback(() => {
        return (_jsx(View, { style: [styles.emptyContainer, { minHeight: Math.round(Math.min(windowHeight * 0.5, 480)) }], children: _jsxs(View, { style: styles.emptyState, children: [_jsx(Text, { style: styles.emptyIcon, children: "\uD83D\uDCD4" }), _jsx(Text, { style: styles.emptyTitle, allowFontScaling: true, maxFontSizeMultiplier: 1.28, children: "Your Journal is Empty" }), _jsx(Text, { style: styles.emptySubtitle, allowFontScaling: true, maxFontSizeMultiplier: 1.35, children: "Start tracking your mood on the Today tab to see your entries here" })] }) }));
    ...
    const renderListEmpty = useCallback(() => {
        if (!journalLoadCompleted) {
            return (_jsx(View, { style: [styles.loadingEmpty, { minHeight: Math.round(Math.min(windowHeight * 0.35, 220)) }], children: _jsx(ActivityIndicator, { color: s.secondaryLabel }) }));
        }
        return renderEmptyState();
    }, [journalLoadCompleted, renderEmptyState, s.secondaryLabel, styles.loadingEmpty, windowHeight]);
    const listContentStyle = useMemo(() => [styles.listContent, { flexGrow: 1 }], [styles.listContent]);
    ...
    return (_jsxs(SafeAreaView, { style: styles.container, edges: ['top'], children: [_jsx(ScreenHeader, { title: "Journal", contentPaddingHorizontal: screenHeaderPrimaryTabPaddingX }), _jsx(PerfProfiler, { id: "list.journal", children: isGroupedView ? (_jsx(SectionList, { sections: journalSectionsForList, keyExtractor: keyExtractor, renderItem: renderEntry, renderSectionHeader: renderJournalSectionHeader, ListHeaderComponent: renderSortHeader, ListEmptyComponent: renderListEmpty, style: listSurfaceStyle, contentContainerStyle: listContentStyle, showsVerticalScrollIndicator: false, stickySectionHeadersEnabled: false, ...scrollListProps, extraData: journalListExtraData }, viewMode === 'byMood'
                    ? `journal-mood-${byMoodSoloGrade ?? 'all'}`
                    : viewMode === 'byDay'
                        ? `journal-weekday-${byDaySoloWeekday ?? 'all'}`
                        : viewMode === 'byMonth'
                            ? `journal-month-${byMonthSoloKey ?? 'all'}`
                            : `journal-${viewMode}`)) : JOURNAL_LIST_IMPL === 'flashlist' ? (
                /**
                 ...
                 */
                _jsx(FlashList, { data: flatEntries, keyExtractor: keyExtractor, renderItem: renderEntry, ListHeaderComponent: renderSortHeader, ListEmptyComponent: renderListEmpty, style: listSurfaceStyle, contentContainerStyle: listContentStyle, drawDistance: 500, showsVerticalScrollIndicator: false, removeClippedSubviews: true, ...scrollListProps, extraData: journalListExtraData })) : (_jsx(FlatList, { data: flatEntries, keyExtractor: keyExtractor, renderItem: renderEntry, ListHeaderComponent: renderSortHeader, ListEmptyComponent: renderListEmpty, style: listSurfaceStyle, contentContainerStyle: listContentStyle, showsVerticalScrollIndicator: false, removeClippedSubviews: true, initialNumToRender: 12, windowSize: 7, maxToRenderPerBatch: 12, updateCellsBatchingPeriod: 50, ...scrollListProps, extraData: journalListExtraData })) }), _jsx(JournalEditModal, { editingEntry: editingEntry, editMood: editMood, editNote: editNote, setEditMood: setEditMood, setEditNote: setEditNote, onCancel: handleCloseEdit, onSave: handleSaveEdit })] }));
}
