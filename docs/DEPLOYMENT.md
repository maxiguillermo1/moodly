# Moodly — production deployment guide

Deploy Moodly as a **standalone** iOS / Android app via **EAS Build**. Core journaling is **local-first** (no backend, no required `.env`).
...
| Apple Developer Program | For App Store / TestFlight (`com.moodly.app`) |
| Google Play Console | For Play Store (`com.moodly.app`) |

### Expo Go (local dev)

Moodly uses **classic** `App.tsx` entry (`expo/AppEntry.js`), **not** Expo Router. Bootstrap lives in **`src/bootstrap/`** (do not recreate a `src/app/` folder — Expo CLI will treat it as Expo Router and Expo Go will fail to open the project).
...
- **Project path must not contain spaces** (e.g. avoid `Desktop/mobile applications/moodly`). Expo Go on a phone hangs or errors otherwise. Fix: `npm run fix:dev-path` (moves to `~/Desktop/moodly`) then `npm run start:clear`.
...
**Not applicable.** Moodly has **no login**, **no tokens**, and **no Moodly cloud account**.  
...
URL scheme: **`moodly://`** (see `scheme` in `app.config.ts`). No universal links configured in the foundation slice.
