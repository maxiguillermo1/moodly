...
**Applies to:** Moodly mobile app (iOS / Android), version 0.6.x and later unless superseded.

## Summary

Moodly is **local-first**. Your mood entries, journal notes, habits, goals, and reminders are stored **on your device** unless you explicitly export or back them up using tools outside the app (for example, an OS-level device backup).

Moodly **does not**:

- Require an account or login for core use
- Operate a Moodly-owned cloud backend for your journal content
...
We do not receive this content on Moodly servers because **there is no Moodly server** for core journaling in the current product.
...
- **Device backup:** iOS iCloud / Android backup may copy app data according to OS rules — that is outside Moodly’s control.

## Children

Moodly is not directed at children under 13. Do not use the app if you are under the age required by your jurisdiction without guardian consent.
...
Open a GitHub issue on the Moodly repository or use the **Support** link in Settings (when configured for your build).
